...
Потому, что шаблонизатор BEMHTML предполагает поведение по умолчанию. Если написать BEMJSON
    ...
    и не написать никаких шаблонов, то в результате сгенерируется такой HTML:
...
обявлены в BEMJSON, например когда сущности обьявляются в шаблонах или шаблоны генерятся
...
выведем список пользователей и их изображения, входящие в организацию BEM на github.com.
    ...
    необходимо это явно указать в файле с зависомостями `bemembers.deps.js`, что технология
    ...
    Меняем в BEMJSON обявление блока `git-members` на блок `bemembers` без параметров и проверяем работу.
