...
Давайте попробуем его создать, а заодно разберемся как оно работает, что есть мифы, а что реальность.
...
к Я.Субботнику скопилось немно косвенной информации. И как часто случается, ничего из этого в доклад не попало,
а добро пропасть не должно.

Хочу заранее предупредить, все технологии рассматриваются поверхностно, и для целей практического использования
...
Чтоб не писать о настройке DNS, на __server__ добавим в `/etc/hosts`:
    ...
    192.168.9.2     gentoo01
    192.168.9.5     gentoo02-gw
    192.168.9.6     gentoo02
    192.168.9.9     ubuntu01-gw
    192.168.9.10    ubuntu01
    192.168.9.13    ubuntu02-gw
    192.168.9.14    ubuntu02
