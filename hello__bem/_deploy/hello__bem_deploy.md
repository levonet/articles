...
Нам дано условное техническое задание — сверстать две статические страницы:
  ...
  - ожидается высокая посещаемость страницы, по этому необходимо оптимизировать выдачу
...
Перезапускаме nginx командой `sudo service nginx reload` и на этот раз, для проверки, в браузере вбиваем имя хоста сервера. Получаем удовольствие, делемся радостью с окружающими.

Подводя промежуточный итог, скопируем nginx-конфиг `/etc/nginx/sites-available/10-hypnotoad.conf` в папку проекта, в `./nginx/sites-available/10-hypnotoad.conf`. Все последующие изменения рекомендую вносить в конфигурационные файлы в проекте, после переносить файлы их в `/etc` сервера.
...
PS: Описанные в статье решения ни есть единственно верные, просто автор их посчитал более наглядными для демонстрации конфигурации проекта.
